...
func UpdateEvent(w http.ResponseWriter, r *http.Request, cache *cache.Cache) {
	...
	if r.Method == "POST" {
		...
		newDate := jsn.NewDate
		mNewDate, _ := regexp.MatchString(`^20\d{2}-\d{2}-\d{2}`, newDate)
		if mNewDate != true {
			io.WriteString(w, fmt.Sprintf(string(eventerrors.OutNewError("invalid new_date format", "date validation"))))
			return
		}

		//валидация нового месяца
		newMonth, _ := strconv.Atoi(date[5:7])
		if newMonth < 1 || newMonth > 12 {
			io.WriteString(w, fmt.Sprintf(string(eventerrors.OutNewError("invalid new_month format", "month validation"))))
			return
		}

		//валидация нового дня
		newDay, _ := strconv.Atoi(date[8:])
		if newDay < 1 || day > 31 {
			io.WriteString(w, fmt.Sprintf(string(eventerrors.OutNewError("invalid new_day format", "day validation"))))
			return
		}
		...
		//перенос события
		cache.UpdateEvent(date, newDate, id)

		//ответ
		out, err := json.Marshal(models.NewResult(newDate, id, "event rescheduled"))
		if err != nil {
