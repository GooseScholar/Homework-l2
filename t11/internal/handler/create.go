...
func CreateEvent(w http.ResponseWriter, r *http.Request, cache *cache.Cache) {
	...
	if r.Method == "POST" {
		...
		//создание события
		cache.PutEvent(date, id)

		//ответ
		out, err := json.Marshal(models.NewResult(date, id, "event created"))
		if err != nil {
